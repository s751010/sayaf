 ...
 *
 * العقد:
 *   POST { plan_id: "standard", cycle: "monthly"|"yearly", promo_code?: string }
 ...
 *   5. **مفتاح إيقاف** في `site_settings.billing.enabled` يوقف التحصيل فوراً
 *      بلا نشر — والاشتراكات القائمة لا تتأثّر.
 *   6. **حدّ معدّل** خمس فواتير/ساعة لكل مستخدم. لا عدّاد في الذاكرة: الدالة
 *      عابرة وتعمل بنسخ متعدّدة، فعدّاد النسخة يحرس نفسه ويترك الباقي مفتوحاً
 *      (نفس درس §14).
 */
...
/**
 * هل التحصيل مفتوح؟ مفتاح الطوارئ في لوحة المؤسّس.
 *
 * ⚠️ **يسقط إلى `true` عند أي فشل قراءة.** غياب المفتاح أو عطل شبكة لا يجوز
 * أن يمنع تاجراً من الدفع — والإيقاف قرارٌ صريح لا حالةٌ افتراضية.
 */
async function billingOpen(admin: SupabaseClient): Promise<boolean> {
  try {
    const { data } = await admin
      ...
      .eq("key", "billing")
      .limit(1);
    const value = data?.[0]?.value as { enabled?: unknown } | undefined;
...
Deno.serve(async (req) => {
  ...
  const { data: allowed, error: rateErr } = await admin.rpc("billing_rate_hit", {
    p_user: user.id,
    p_limit: RATE_LIMIT,
  });
  if (rateErr) {
  ...
  const label = `اشتراك ${planName(planId)} (${cycleLabel})`;
  ...
  try {
    ...
    return json({
      ...
      // الواجهة تعرض شارة «وضع تجريبي» من هنا لا من ثابت في العميل.
