...
/** هل بيانات اعتماد المنصّة مضبوطة أصلاً؟ (بلا كشف أي جزء منها) */
export function hasPlatformCredentials(): boolean {
  return !!(Deno.env.get("PAYLINK_API_ID") && Deno.env.get("PAYLINK_SECRET_KEY"));
}

/**
 * آخر ثلاثة محارف من **المعرّف** (لا السرّ) — تكفي لتعرف أي حساب موصول،
 * ولا تكفي لانتحاله. تُعرض في لوحة المؤسّس وحدها.
 */
export function apiIdTail(): string {
  const id = Deno.env.get("PAYLINK_API_ID") ?? "";
 ...
 */
export async function authToken(creds?: PaylinkCredentials): Promise<string> {
