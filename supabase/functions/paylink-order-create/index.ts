...
import { restaurantHasActiveSubscription } from "../_shared/subscription.ts";
...
Deno.serve(async (req) => {
  ...
  /**
   * ── بوّابة الاشتراك: الدفع الإلكتروني ميزة مدفوعة ────────────────────
   *
   * ⚠️ **الحارس هنا لا في اللوحة.** `FREE_LIMITS.onlinePayment = false` كان
   * إعلاناً لا فرضاً: صفر مكوّن يقرؤه وصفر تريجر يفرضه، فتاجرٌ انتهى اشتراكه
   * يبقى يقبض بالبطاقة داخل منيوه إلى الأبد. والزبون يصل هذه النقطة من صفحة
   * منيو عامّة — لا شاشة لوحة بيننا وبينه تُخفي الزرّ.
   *
   * و**قبل** قراءة بيانات الاعتماد عمداً: لا يُلمس `secret_key` لمن لا يحقّ
   * له المسار أصلاً.
   *
   * ⚠️ ولا يُمسّ ما دونها: المنيو والسلّة **وطلبات واتساب** مجانية بقرار سوقي
   * مكتوب (`FREE_LIMITS.cart = true`) — المدفوع هو الدفع بالبطاقة لا الطلب.
   * فالرسالة تدلّ على البديل بدل أن تُغلق الباب.
   */
  if (!(await restaurantHasActiveSubscription(admin, restaurantId))) {
    return json(
      {
        error: "الدفع الإلكتروني غير متاح حالياً — أرسل طلبك عبر واتساب.",
