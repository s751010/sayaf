...
export function signOut() {
  /**
   * ⚠️ **الرمز يُبطَل عند الخادم أيضاً — لا محلياً وحده.**
   *
   * كان التعليق هنا يقول «إسقاط الرموز محلياً يكفي لـSPA»، وهو صحيح لواجهة
   * المستخدم وخاطئ أمنياً: `refresh_token` يبقى **صالحاً عند GoTrue** بعد
   * الخروج. والجلسة تعيش في `localStorage` (قرار موثَّق، وCSP مخفّفه الوحيد)،
   * فرمزٌ سُرّب بـXSS يظلّ يُجدِّد وصولاً بعد أن يخرج صاحبه ويظنّ أنه أغلق
   * الباب — وهو بالضبط ما يفعله المستخدم حين يشكّ أن حسابه اختُرق.
   *
   * والنداء **قبل** المسح المحلّي لأنه يحتاج الرمز، و`keepalive` ليمضي حتى
   * لو أُغلق التبويب في نفس اللحظة.
   */
  const s = loadSession();
  if (s?.access_token) {
    void fetch(`${SUPABASE_URL}/auth/v1/logout`, {
      ...
      keepalive: true,
      // فشل الإبطال لا يمنع الخروج: المحلّي يُمسح في الحالتين، وإلا عَلِق
      // المستخدم داخل حسابه لأن الشبكة تعطّلت.
    }).catch(() => {});
  }
  clearSession();
}
