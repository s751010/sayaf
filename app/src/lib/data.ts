 ...
 *
 * ═══ ⚠️ هذا الرقم لا يفرض شيئاً ═══
 *
 * المدّة الفعليّة تحسبها القاعدة في `guard_client_subscription` من
 * `site_settings.billing.trial_days` (عبر دالة `trial_days()`)، وهي **تدهس**
 * أي `end_date` يصل من المتصفح. فهذا الثابت احتياطٌ للعرض في أول رسمة قبل
 * أن تصل الإعدادات، لا أكثر — استعمل `trialDays()` أدناه للعرض الحقيقي.
 *
 * وكان الرقمان مصدرين متنافسين: الواجهة تحسب `end_date` وترسله، والتريجر
...
export async function trialDays(): Promise<number> {
  try {
    const settings = await getSiteSetting<{ trial_days?: number }>("billing");
    const n = Number(settings?.trial_days);
    return Number.isInteger(n) && n > 0 ? n : TRIAL_DAYS;
  } catch {
    return TRIAL_DAYS;
  }
}
 ...
 *
 * لا يُرسل `end_date` ولا `active`: التريجر `guard_client_subscription`
 * يضبطهما بنفسه. إرسالهما من هنا كان يجعل المتصفح شريكاً في قرار المدّة —
 * وهو ما فتح باب التلاعب أصلاً.
 *
 * `is_menu_published` تسأل عن اشتراك نشط لم ينتهِ فقط، فصفّ التجربة يفتح
 * نشر المنيو تلقائياً.
 *
 * فشل الإنشاء لا يُوقف التسجيل: التريجر يرفض تجربة ثانية لنفس المستخدم،
 * وهذا رفض متوقّع لا خطأ يستحق إزعاج التاجر به.
 */
export async function startTrial(userId: string): Promise<void> {
  await rest("subscriptions", {
    method: "POST",
    headers: { Prefer: "return=minimal" },
    body: { user_id: userId, plan_id: "trial" },
  });
}
