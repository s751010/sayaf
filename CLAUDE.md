...
> ⚠️ **اقرأ شرطها قبل أن تخافها.** كُتب بعضها بحجّة «١٨ مطعماً حقيقياً عليها»،
> وتلك أرقام المشروع القديم (`wjqpsbpebpntpeinqccl`) ولم تنتقل: القاعدة الحيّة
> اليوم فيها **٣ مطاعم وصفر اشتراك مدفوع** (٢٣ أغسطس ٢٠٢٦).
>
> فالقاعدة الصحيحة: هذه ثوابت **بعد أوّل تاجر مدفوع**. وقبله هي *تفضيلات
> قويّة* — تُكسر بقرارٍ مكتوب لا بسهو، ويُحدَّث فحصها معها. وأن تُعامَل قبل
> ذلك معاملةَ عقدٍ مع جمهورٍ غير موجود يشتري لك جموداً بلا مقابل.
>
> والاستثناء الحقيقي الذي يبقى قبل أوّل تاجر: **كل ما دخل مطبوعاً** — روابط
> المنيو وأكواد QR والبطاقات. المطبوع لا يُحدَّث.

| الثابت | لماذا | أين تفصيله |
|---|---|---|
| **قيم ألوان الطوابع لا تتغيّر** | `dark-gold` شكلُ المنتج لمن لم يختر — والبصمة تمنع انحرافاً صامتاً | §18 · `invariants.test.ts` (بصمة) |
...
| **الباقة الواحدة `standard` — والسعر من `_shared/plans.ts` وحده** | ثلاث نسخ من الجدول تباعدت فعلاً: دالّتان كانتا تنتظران ٩٩ والموقع يعرض ٥٩، ففحص المبلغ يرفض كل دفعة صحيحة. **والسعر اليوم ٩٩ شهرياً / ٥٩٩ سنوياً** | §4 · `parity.test.ts` (لا سعر برقم خارج المصدر) |
...
3ب. `npm run e2e` — ١٤ حالة في كروميوم (حاسوباً وجوالاً). **هذه وحدها تفتح
   متصفّحاً**: الثلاث خطوات فوقها كلّها على دوالّ خالصة، وعطل `React #310`
   الذي أطفأ كل منيو مرّ منها جميعاً.
...
**CI يعيد ٢ و٣ و٣ب و٥ على كل دفعة** (`.github/workflows/ci.yml`) — فالخطوات
أعلاه سرعةٌ لك لا حارسٌ وحيد. وفوقها **نبضة** كل نصف ساعة
(`.github/workflows/canary.yml`) تفحص المنتج **حيّاً**: الموقع، ثم `/demo`،
ثم استعلامٌ عامّ حقيقي — وهذا الأخير يمسك عموداً بلا `grant select` قبل أن
يمسكه تاجر (القاعدة «و»).
...
- **لا حدود عددية أصلاً**: `maxDishes` و`maxMenus` قيمتهما `null` في الطبقتين
  (`FREE_LIMITS` و`ALL_FEATURES`) — أي أن فحص العميل في `Dishes.tsx`/`Menus.tsx`
  لا يمنع شيئاً اليوم. المقابل **أدوات لا أعداد**.
  وأيّ حدٍّ عدديّ يُضاف مستقبلاً يجب أن يُفرض في القاعدة (تريجر) لا في الفورم.
- ✅ **وواجهة API صارت تفحص الاشتراك من الخادم** (٢٣ أغسطس): كانت تفحص المفتاح
  وحدّ المعدّل فقط، فمفتاحٌ أُصدر في التجربة يعمل بعدها إلى الأبد. الآن ٤٠٢
  بلا اشتراك نشط — وهي أوّل ميزة مدفوعة تُحرَس خارج المتصفّح.
