...
- `restaurants` مقروء علناً بالكامل (`qual = true`). هذا مقصود لصفحة المنيو، لكنه
  يعني أن أي شخص يقدر يسرد كل المطاعم وبياناتها. إن أردت تضييقه، الحل أن تُقصر
  القراءة العامة على أعمدة محدّدة عبر view.
- ٦ مطاعم من ١٦ بلا `slug` — صفحاتها العامة لا تُفتح (ظهرت في لوحة صحة النظام).
